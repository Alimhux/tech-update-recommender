# PLAN.md — Tech Update Recommender
...
│   ├── config.py            # Загрузка конфигурации (~/.tech-update-recommender.yaml + env vars)
...
```
tech-update-recommender scan <path> [опции]
...
Конфигурация загружается каскадно: CLI аргументы > env vars > ~/.tech-update-recommender.yaml > defaults.
...
**Конфигурация LLM:**
- API-ключ: CLI-аргумент > env var (TUR_LLM_API_KEY, или стандартные ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY) > ~/.tech-update-recommender.yaml
...
Путь: `~/.tech-update-recommender.yaml`
...
cache:
  ...
  path: "~/.cache/tech_update_recommender/"
